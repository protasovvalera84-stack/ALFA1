...
(function initGSAP() {
  ...
  const serviceGrid = document.querySelector('.grid.sm\\:grid-cols-2.lg\\:grid-cols-3.xl\\:grid-cols-4');
  if (serviceGrid) {
    gsap.from(serviceGrid.querySelectorAll('.service-card'), {
      y: 40, opacity: 0, duration: 0.5, stagger: 0.06, ease: 'power2.out',
      scrollTrigger: { trigger: serviceGrid, start: 'top 80%', toggleActions: 'play none none none' },
    });
  }
  ...
  gsap.utils.toArray('.stat-card').forEach((card, i) => {
    gsap.from(card, {
      scale: 0.85, opacity: 0, duration: 0.6, delay: i * 0.08, ease: 'back.out(1.5)',
  ...
  gsap.utils.toArray('.timeline-item').forEach((item, i) => {
    gsap.from(item, {
      x: i % 2 === 0 ? -50 : 50, opacity: 0, duration: 0.75, ease: 'power3.out',
  ...
  gsap.utils.toArray('.contact-info-item').forEach((item, i) => {
    gsap.from(item, {
      x: -30, opacity: 0, duration: 0.45, delay: i * 0.08, ease: 'power2.out',
...
(function initContactForm() {
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      ...
      setTimeout(() => submitBtn.classList.remove('ring-2', 'ring-gold', 'ring-offset-2', 'ring-offset-bg-secondary'), 2000);
...
(function initBackToTop() {
  ...
  btn.style.cssText = [
    ...
    'background:rgba(212,175,55,0.15)', 'backdrop-filter:blur(12px)',
    'border:1px solid rgba(212,175,55,0.35)',
    'border-radius:0.75rem', 'color:#D4AF37',
  ...
  btn.addEventListener('mouseleave', () => { btn.style.background = 'rgba(212,175,55,0.15)'; });
})();
