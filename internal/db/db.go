...
func migrate(db *sql.DB) error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS clients (
			...
			color_class TEXT    NOT NULL DEFAULT 'gold',
...
func seedDefaults(db *sql.DB) error {
	...
	if count > 0 {
		// Only seed new tables that may be empty on existing installs.
		if err := seedNewTables(db); err != nil {
			return fmt.Errorf("db: seed new tables: %w", err)
		}
		return nil
	}
...
// seedNewTables seeds the 7 new content tables if they are empty.
// Safe to call on every startup — uses INSERT OR IGNORE.
func seedNewTables(db *sql.DB) error {
	// ── Advantages ─────────────────────────────────────────────────────────
	var cnt int
	if err := db.QueryRow(`SELECT COUNT(*) FROM advantages`).Scan(&cnt); err != nil {
		return err
	}
	if cnt == 0 {
		advantages := []struct{ title, desc string }{
		...
		for i, a := range advantages {
			if _, err := db.Exec(`INSERT OR IGNORE INTO advantages (title, description, sort_order) VALUES (?, ?, ?)`, a.title, a.desc, i); err != nil {
				return err
			}
		}
	}
	...
	// ── History events ─────────────────────────────────────────────────────
	if err := db.QueryRow(`SELECT COUNT(*) FROM history_events`).Scan(&cnt); err != nil {
		return err
	}
	if cnt == 0 {
		events := []struct{ year, title, desc, quote string }{
			...
			{"1985–1995", "Рота оперативного реагирования (РОР)", "Преемник олимпийской группы — Рота оперативного реагирования. Специализация: задержание вооружённых и особо опасных преступников. Десятки успешных операций по всему Северо-Западу.", "Рота специализировалась на задержании вооружённых и особо опасных преступников — живыми и без применения оружия."},
			{"1990-е", "Формирование Резерва и СОБР", "На базе РОР сформирован Резерв — подразделение для выполнения задач наивысшей сложности. Впоследствии преобразован в самостоятельный Специальный отряд быстрого реагирования (СОБР).", "Ветераны СОБР и РОР стали основой нового охранного предприятия, принеся с собой уникальный боевой опыт."},
			{"2002", "Основание ЧОО «Альфа Юнит-1»", "Ветераны элитных силовых структур создали частное охранное предприятие в Санкт-Петербурге. С первых дней компания объединила высочайший профессионализм силовых структур с гибкостью коммерческой организации.", "Год основания группы компаний «Альфа Юнит-1»"},
		...
		for i, e := range events {
			if _, err := db.Exec(`INSERT OR IGNORE INTO history_events (year_label, title, description, quote, sort_order) VALUES (?, ?, ?, ?, ?)`, e.year, e.title, e.desc, e.quote, i); err != nil {
				return err
			}
		}
	}
	...
	// ── Licenses ───────────────────────────────────────────────────────────
	if err := db.QueryRow(`SELECT COUNT(*) FROM licenses`).Scan(&cnt); err != nil {
		return err
	}
	if cnt == 0 {
		licenses := []struct{ typeLabel, title, desc, badge string }{
		...
		for i, l := range licenses {
			if _, err := db.Exec(`INSERT OR IGNORE INTO licenses (type_label, title, description, badge_label, sort_order) VALUES (?, ?, ?, ?, ?)`, l.typeLabel, l.title, l.desc, l.badge, i); err != nil {
				return err
			}
		}
	}
	...
	// ── Clients ────────────────────────────────────────────────────────────
	if err := db.QueryRow(`SELECT COUNT(*) FROM clients`).Scan(&cnt); err != nil {
		return err
	}
	if cnt == 0 {
		clients := []struct{ letter, color, name, typeLabel, desc, tags string }{
			{"С", "gold", "«Советская Звезда»", "Коммерческая недвижимость", "Охрана объектов коммерческой недвижимости в Адмиралтейском районе Санкт-Петербурга. Контроль доступа арендаторов и посетителей.", "Бизнес-центр,КПП"},
			{"Т", "blue", "АО «Трест № 68»", "Строительное предприятие", "Охрана строительного предприятия, включающего 4 производственных подразделения. Круглосуточный контроль периметра и въезда техники.", "Строительство,Периметр,24/7"},
			{"В", "gold", "«ВК Сервис»", "Инфраструктурный объект", "Охрана объектов компании, специализирующейся на очистке водоотведения и диагностике трубопроводов. Охрана оборудования и складов.", "Инфраструктура,Склад"},
			{"К", "blue", "Гостиница «Северная Корона»", "Строительная площадка", "Охрана строительной площадки гостиницы в Санкт-Петербурге. Контроль материалов, техники и персонала на всех этапах строительства.", "Стройплощадка,Материалы"},
			{"Х", "gold", "«Технология холода»", "Холодильные склады", "Охрана холодильных складских комплексов и арендуемых помещений. Контроль въезда транспорта, сохранность продукции.", "Склад,Холодильник,Въезд/выезд"},
			{"П", "blue", "Ведомственная парковка", "Тележная ул., д. 32", "Охрана ведомственной парковки: контроль въезда/выезда транспортных средств, видеонаблюдение, пропускной режим.", "Парковка,Видеонаблюдение"},
			{"А", "gold", "ГК «Алгоритм»", "Демонтажные работы", "Охрана объектов группы компаний, специализирующейся на демонтажных работах. Обеспечение безопасности строительных площадок.", "Демонтаж,Строительство"},
			{"R", "blue", "«Ренейссанс Констракшн»", "Международная строительная компания", "Охрана объектов международной строительной компании в Санкт-Петербурге. Многоуровневая система безопасности крупных стройплощадок.", "Стройплощадка,Международный"},
			{"М", "gold", "«Медиэстетик»", "Клиника эстетической медицины", "Охрана клиники эстетической медицины: обеспечение безопасности пациентов и персонала, контроль доступа в медицинское учреждение.", "Медицина,Клиника"},
			{"Э", "blue", "ООО «ЭнергоСвязьСтрой»", "Электросетевая инфраструктура", "Охрана объектов компании, проектирующей электросетевую инфраструктуру от 0,4 до 750 кВ. Защита проектной документации и оборудования.", "Энергетика,Офис,Оборудование"},
			{"В", "gold", "«Венчурный Капитал»", "Инвестиционная компания", "Физическая охрана офиса инвестиционной компании, обеспечение конфиденциальности переговоров и защита ценных активов.", "Офис,Конфиденциальность"},
		}
		for i, c := range clients {
			if _, err := db.Exec(`INSERT OR IGNORE INTO clients (letter, color_class, name, type_label, description, tags, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)`, c.letter, c.color, c.name, c.typeLabel, c.desc, c.tags, i); err != nil {
				return err
			}
		}
	}
	...
	// ── Team members ───────────────────────────────────────────────────────
	if err := db.QueryRow(`SELECT COUNT(*) FROM team_members`).Scan(&cnt); err != nil {
		return err
	}
	if cnt == 0 {
		members := []struct{ letter, color, title, dept, desc, tags string }{
			{"А", "gold", "Генеральный директор", "ЧОО «Альфа Юнит-1»", "Ветеран силовых структур. Руководит группой компаний с момента основания в 2002 году. Свыше 25 лет опыта в сфере безопасности.", "Стратегия,Управление,Ветеран «Альфа»"},
			{"О", "blue", "Начальник охраны", "Оперативный отдел", "Ветеран СОБР. Организация охранных мероприятий, тактическая подготовка личного состава. 20 лет боевого и оперативного опыта.", "Тактика,СОБР,Подготовка"},
			{"К", "gold", "Руководитель HR", "Кадровая служба", "Отбор и аттестация сотрудников. Организация медицинских комиссий, квалификационных экзаменов в Росгвардии, физподготовки.", "Кадры,Аттестация"},
		}
		for i, m := range members {
			if _, err := db.Exec(`INSERT OR IGNORE INTO team_members (letter, color_class, title, department, description, tags, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)`, m.letter, m.color, m.title, m.dept, m.desc, m.tags, i); err != nil {
				return err
			}
		}
	}
	...
	// ── Process steps ──────────────────────────────────────────────────────
	if err := db.QueryRow(`SELECT COUNT(*) FROM process_steps`).Scan(&cnt); err != nil {
		return err
	}
	if cnt == 0 {
		...
		for i, s := range steps {
			if _, err := db.Exec(`INSERT OR IGNORE INTO process_steps (step_num, title, description, sort_order) VALUES (?, ?, ?, ?)`, s.num, s.title, s.desc, i); err != nil {
				return err
			}
		}
	}
	...
	// ── FAQ ────────────────────────────────────────────────────────────────
	if err := db.QueryRow(`SELECT COUNT(*) FROM faqs`).Scan(&cnt); err != nil {
		return err
	}
	if cnt == 0 {
		...
		for i, f := range faqs {
			if _, err := db.Exec(`INSERT OR IGNORE INTO faqs (question, answer, sort_order) VALUES (?, ?, ?)`, f.q, f.a, i); err != nil {
				return err
			}
		}
	}
