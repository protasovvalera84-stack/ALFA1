...
func seedDefaults(db *sql.DB) error {
	...
	// Seed history items.
	historyItems := []struct{ year, title, subtitle, description, quote string }{
		{"1980", "Олимпийская группа захвата", "Создана специальная группа захвата", "Создана специальная группа захвата для обеспечения безопасности в период проведения Олимпийских игр в Москве. Одно из самых подготовленных и профессиональных подразделений МВД СССР.", "Десятки блестяще проведённых операций — задержание вооружённых и особо опасных преступников живыми, без применения оружия"},
		{"1985–1995", "Рота оперативного реагирования (РОР)", "Преемник олимпийской группы", "Преемник олимпийской группы — Рота оперативного реагирования. Специализация: задержание вооружённых и особо опасных преступников. Десятки успешных операций по всему Северо-Западу.", ""},
		{"1990-е", "Формирование Резерва и СОБР", "На базе РОР сформирован Резерв", "На базе РОР сформирован Резерв — подразделение для выполнения задач наивысшей сложности. Впоследствии преобразован в самостоятельный Специальный отряд быстрого реагирования (СОБР).", ""},
		{"2002", "Основание ЧОО «Альфа Юнит-1»", "Год основания группы компаний", "Ветераны элитных силовых структур создали частное охранное предприятие в Санкт-Петербурге. С первых дней компания объединила высочайший профессионализм силовых структур с гибкостью коммерческой организации.", ""},
		{"Сегодня", "Группа компаний — лидер СЗФО", "Два юридических лица", "Два юридических лица (ЧОО «Альфа Юнит-1» и ЧОО «Альфа Безопасность»), более 50+ объектов под охраной, 200+ сотрудников. Офисы в Санкт-Петербурге и Симферополе.", ""},
	}
	for i, h := range historyItems {
		if _, err := db.Exec(
			`INSERT OR IGNORE INTO history_items (year, title, subtitle, description, quote, sort_order) VALUES (?, ?, ?, ?, ?, ?)`,
			h.year, h.title, h.subtitle, h.description, h.quote, i,
		); err != nil {
			return err
		}
	}
	...
	// Seed licenses.
	licenseItems := []struct{ title, subtitle, desc, badge string }{
		{"Лицензия ЧОО «Альфа Юнит-1»", "Лицензия на осуществление частной охранной деятельности ЧОО «Альфа Юнит-1». Санкт-Петербург и СЗФО.", "Лицензия на осуществление частной охранной деятельности.", "Действующая лицензия"},
		{"Лицензия ЧОО «Альфа Безопасность»", "Лицензия на осуществление частной охранной деятельности ЧОО «Альфа Безопасность». Санкт-Петербург и СЗФО.", "Лицензия на осуществление частной охранной деятельности.", "Действующая лицензия"},
		{"Свидетельство Ассоциация «Альфа»", "Свидетельство о членстве в Международной ассоциации ветеранов подразделения «АЛЬФА» — знак высокого профессионализма.", "Свидетельство о членстве в Международной ассоциации ветеранов.", "Действующее свидетельство"},
	}
	for i, l := range licenseItems {
		if _, err := db.Exec(
			`INSERT OR IGNORE INTO licenses (title, subtitle, description, badge, sort_order) VALUES (?, ?, ?, ?, ?)`,
			l.title, l.subtitle, l.desc, l.badge, i,
		); err != nil {
			return err
		}
	}
	...
	// Seed clients.
	clients := []struct{ letter, name, sector, desc, tags string }{
	...
	for i, c := range clients {
		if _, err := db.Exec(
			`INSERT OR IGNORE INTO clients (letter, name, sector, description, tags, sort_order) VALUES (?, ?, ?, ?, ?, ?)`,
			c.letter, c.name, c.sector, c.desc, c.tags, i,
		); err != nil {
			return err
		}
	}
	...
	// Seed team members.
	teamMembers := []struct{ letter, name, role, dept, desc, tags string }{
	...
	for i, t := range teamMembers {
		if _, err := db.Exec(
			`INSERT OR IGNORE INTO team_members (letter, name, role, department, description, tags, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)`,
			t.letter, t.name, t.role, t.dept, t.desc, t.tags, i,
		); err != nil {
			return err
		}
	}
	...
	// Seed process steps.
	processSteps := []struct{ num, title, desc string }{
	...
	// Seed FAQ items.
	faqItems := []struct{ question, answer string }{
		{"Сколько стоят услуги охраны?", "Стоимость рассчитывается индивидуально в зависимости от типа объекта, его площади, режима охраны и других факторов. Позвоните нам — специалист проведёт бесплатный анализ объекта и подготовит коммерческое предложение."},
		{"Есть ли у компании лицензия на вооружённую охрану?", "Да, группа компаний имеет все необходимые лицензии на осуществление частной охранной деятельности, в том числе с использованием специальных средств и огнестрельного оружия."},
		{"Как быстро можно приступить к охране объекта?", "В стандартных случаях — в течение 2–5 рабочих дней после подписания договора. При срочной необходимости возможна более оперативная расстановка персонала."},
		{"В каких регионах вы работаете?", "Основная деятельность — Санкт-Петербург и Северо-Западный федеральный округ. Также работаем в Симферополе и по договорённости в других регионах России."},
		{"Что входит в охранную сигнализацию?", "Проектирование, поставку и монтаж охранной сигнализации, тревожных кнопок и систем видеонаблюдения. После установки обеспечиваем обслуживание и оперативное реагирование."},
		{"Как трудоустроены сотрудники охраны?", "Все сотрудники официально трудоустроены, имеют удостоверение частного охранника установленного образца, прошли медицинскую комиссию и квалификационный экзамен в Росгвардии."},
	}
	for i, f := range faqItems {
		if _, err := db.Exec(
